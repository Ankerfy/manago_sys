...
#### 步骤 1：先确保你已创建 Pinia Store（假设为 `ui-store.js`）
...
| 路由中调用 | ❌ 无法使用 `useProgress`，需绕道 `utils`    | ✅ `useUIStore().start()` |
...
| `@/stores/ui-store.js`         | **新建**（内容见上）                      |
| `@/router/index.js`            | 改为 `useUIStore().start(...)`            |
